...
expensesBtn.addEventListener('click', function() {
    ...
    for(let i = 0; i < expensesItem.length ; i++) {
        let expen = expensesItem[i].value,
...
countBtn.addEventListener('click', function(){
    ...
    if(appData.budget != undefined) {
        ...
        if(appData.moneyPerDay < 100 ) {
            levelValue.textContent = 'Мин доход';
        } else if(appData.moneyPerDay > 100 && appData.moneyPerDay < 2000) {
            levelValue.textContent = 'Ср доход';
...
optionalExpensesBtn.addEventListener('click', function(){
    for(let i = 0; i < 3; i++) {
        let binExpenses = optionalExpensesItem[i].value;
...
//  document.getElementsByClassName('expenses-item-btn').disavled = false;
